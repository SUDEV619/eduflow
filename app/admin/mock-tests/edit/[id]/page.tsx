...
import { 
  ...
  RotateCcw, Layers, Zap, X, Trash2, Loader2
} from 'lucide-react'
import { useAuth } from '../../../../context/AuthContext'
import { adminApi, Question, MockTest } from '../../services/api'
...
export default function EditMockTest() {
  ...
  useEffect(() => {
    const fetchTestData = async () => {
      ...
      try {
        const testIdNum = parseInt(id)
        if (isNaN(testIdNum)) {
          router.push('/admin/mock-tests')
          return
        }
        
        const res = await adminApi.getMockTestDetail(token, testIdNum)
  ...
  const handleSubmit = async (publish: boolean) => {
    ...
    try {
      const testIdNum = parseInt(id)
      // 1. Update Test Info
      const testRes = await adminApi.updateMockTest(token, testIdNum, {
        ...testInfo,
        is_published: publish
      })
      
      if (testRes.status === 'success') {
        // 2. Update Questions (using setQuestionsInTest)
        try {
          await adminApi.setQuestionsInTest(token, testIdNum, selectedQuestions.map(q => q.id))
        } catch (qErr: any) {
          console.error('Failed to update questions:', qErr)
          alert(`Test info updated, but failed to update questions: ${qErr.message}`)
        }
  ...
  if (isDataLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] text-[#4A465F]/40">
        ...
        <p className="font-bold">Loading test details...</p>
      </div>
    )
  }
    ...
    <div className="max-w-6xl mx-auto space-y-8 pb-20">
        ...
        {step === 1 && (
          <div className="space-y-8 max-w-2xl mx-auto w-full">
            <div className="flex items-center gap-4 mb-2">
              ...
              <div>
                <h2 className="text-2xl font-bold text-[#4A465F]">Edit Test Configuration</h2>
                <p className="text-sm text-[#4A465F]/60">Update the core settings for this assessment</p>
              </div>
            </div>
          ...
          <div className="h-full flex flex-col space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-[#4A465F]">Edit Question Bank</h2>
      ...
      <div className="flex justify-between items-center bg-white/80 backdrop-blur-md p-6 rounded-[2rem] border border-[#4A465F]/5 shadow-lg shadow-[#4A465F]/5">
        ...
        >
          ...
          {step === 1 ? 'Cancel Edits' : 'Previous Step'}
        </button>
